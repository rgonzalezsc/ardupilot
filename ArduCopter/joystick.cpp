...
uint8_t roll_pitch_flag = true; // Flag to adjust roll/pitch instead of forward/lateral - Necesario true para avion/quad
}

void Copter::init_joystick()
{
    default_js_buttons();

    //set_mode(MANUAL, MODE_REASON_TX_COMMAND); // Initialize flight mode
    set_mode(STABILIZE, MODE_REASON_TX_COMMAND); // Initialize flight mode
...
void Copter::transform_manual_control_to_rc_override(int16_t x, int16_t y, int16_t z, int16_t r, uint16_t buttons)
{
    ...
    channels[2] = constrain_int16((z+0)*0.8f+1100,1100,1900); // throttle
    ...
    channels[6] = 0;             // Unused
    channels[7] = 0;//cam_tilt;      // camera tilt
    channels[8] = 0;//lights1;       // lights 1
    channels[9] = 0;//lights2;       // lights 2
    channels[10] = 0;//video_switch; // video switch
...
void Copter::handle_jsbutton_press(uint8_t button, bool shift, bool held)
{
    ...
    /*case JSButton::button_function_t::k_mode_manual: // En copter no tenemos modo MANUAL (= salida directa a servos)
...
JSButton* Copter::get_button(uint8_t index)
{
...
void Copter::default_js_buttons()
{
...
void Copter::set_neutral_controls()
{
    int16_t channels[11];
